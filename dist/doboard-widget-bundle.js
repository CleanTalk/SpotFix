...
function putHandler(item, store, e) {
    if (item?.user_id || item?.task_id || item?.comment_id) {
...
const LOCAL_DATA_BASE_TABLE = [
    ...
    {name: TABLE_TASKS, keyPath: 'task_id'},
    {name: TABLE_COMMENTS, keyPath: 'comment_id'},
];
console.log(3);
    ...
    init: async () => {
        ...
        return new Promise((resolve) => {
            if (!localStorage.getItem('spotfix_session_id')) {
                ...
                const indexedDB = window.indexedDB.open(indexedDBName, indexedDBVersion);
                indexedDB.onupgradeneeded = (e) => {
                    const db = e.target.result;
                    LOCAL_DATA_BASE_TABLE.forEach((item) => {
                        if (!db.objectStoreNames.contains(item.name)) {
                            let tempStore = db.createObjectStore(item.name, {keyPath: item.keyPath});
                            if (item.name === TABLE_COMMENTS) {
                                tempStore.createIndex('task_id', 'task_id');
                            }
                        }
                    });
                    resolve({
